...
This document outlines how the functions in the OCSdata package should be used to access data from [Open Case Studies](https://www.opencasestudies.org/). The [arguments](#Arguments) section explains all of the inputs necessary for the functions to run and examples of appropriate inputs are given. The 'how to use' section explains the purpose of the functions and gives an example for each. The functions are also connected to the corresponding case study section when applicable. 
...
$\color{red}{\text{Note: To download the data into your current working directory, change the `outpath` argument to `getwd()`.}}$ 

$\color{blue}{\text{Just change "ocs-bp-opioid-rural-urban" to the ID of the case study you are working on! (See above for list of IDs)}}$ 
